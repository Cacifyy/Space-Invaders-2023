...
/*
    ...
    if (Cconis()) {
        /*handle menu selection, I forget how to make the game stop and wait.

    }
*/
    render_master(base, &laser_cannon, &laser, &invader, &score);

    while((Player_loses(game_over) == FALSE)) {
        ...
        if (time > 0)
        {      
            sync_ev(&invader, &laser, &score, &laser_cannon);
...
void sync_ev (Invader *invader, Laser *laser, Score *score , Laser_Cannon *laser_cannon ) {
    ...
    hit_det_on_player(laser_cannon, laser);
    /*
